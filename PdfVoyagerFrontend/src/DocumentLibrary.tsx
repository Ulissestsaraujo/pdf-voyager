...
const DocumentLibrary = () => {
    ...
    useEffect(() => {
        const fetchPdfs = async () => {
            try {
                const { data } = await api.get("/api/pdf/user/123")
                setPdfs(data);
            } catch (error) {
    ...
    return (
        <div className="container mx-auto p-6">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">Document Library</h1>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {pdfs.map((pdf) => (
                    ...
                    >
                        <h3 className="font-semibold text-lg text-gray-800 truncate">{pdf.fileName}</h3>
