...
const PdfViewer = () => {
    ...
    const [renderedPages, setRenderedPages] = useState<{ [key: number]: boolean }>({});
    ...
    useEffect(() => {
        const fetchReadSasUrl = async () => {
            try {
                const response = await api.get(`/api/pdf/123/${pdfId}/read-url`);
    ...
    useEffect(() => {
        ...
        const loadPdf = async () => {
            try {
                ...
                const renderPage = async (pageNum: number, canvasRef: React.RefObject<HTMLCanvasElement | null>) => {
                ...
                if (twoPageView && currentPage + 1 <= pdf.numPages && canvasRefs[1].current) {
                    await renderPage(currentPage + 1, canvasRefs[1]);
                } else if (canvasRefs[1].current) {

                    const context = canvasRefs[1].current.getContext("2d");
                    if (context) {
                        context.clearRect(0, 0, canvasRefs[1].current.width, canvasRefs[1].current.height);
                    }
                }

                if (currentPage + 2 <= pdf.numPages && !renderedPages[currentPage + 2]) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
            <div className="bg-white shadow-lg rounded-lg p-4 w-full max-w-7xl flex flex-col items-center">
                ...
                <div
                    className={`flex ${twoPageView ? "gap-6" : "gap-0"} w-full h-[85vh] items-center justify-center overflow-hidden`}
                >
                    <canvas
                        ref={canvasRefs[0]}
                        className={`shadow-md border border-gray-300 ${twoPageView ? "w-[48%]" : "w-full"
                ...
                <div className="mt-4 flex justify-between items-center w-full px-6">
                    <button
                        onClick={() => setCurrentPage((prev) => Math.max(prev - (twoPageView ? 2 : 1), 1))}
                    ...
                    <button
                        onClick={() => setCurrentPage((prev) => (numPages ? Math.min(prev + (twoPageView ? 2 : 1), numPages) : prev))}
